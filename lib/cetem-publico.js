...
class CETEMPublico {
  ...
  _initInput(log){
    ...
    str.on('progress', state => {
      ...
      const percent    = state.percentage.toFixed(2);
      const elapsed    = humanizeSecs(state.runtime);
      const remaining  = humanizeSecs(state.remaining);
      const transf     = prettyBytes(state.transferred);
      const total      = prettyBytes(state.length);

      if(log){
        process.stdout.write(`Training: ${speed}\t${percent}%\t${elapsed}/${remaining}\t${transf}/${total}\r`);
      }
    });
